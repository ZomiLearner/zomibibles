...
from pythonbible import Book, get_number_of_chapters, get_number_of_verses, get_verse_id
...
# Only take the first 66 books (canonical Protestant Bible)
books = list(Book)[:66]

# Build verse map + print OSIS references
index = 1
verse_map = {}

verses = []
for book_idx, book in enumerate(books):
    code = book_codes[book_idx]
    try:
        for ch in range(1, get_number_of_chapters(book) + 1):
            for vs in range(1, get_number_of_verses(book, ch) + 1):
                vid = get_verse_id(book, ch, vs)
                verse_map[index] = vid
                verses.append(f"{code}_{ch}_{vs}")
                # print(f"{code}_{ch}_{vs}")
                index += 1
    except Exception:
        continue
print(len(verses))
